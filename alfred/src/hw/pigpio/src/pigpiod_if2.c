  ...
  if (r->flags == 0) {
    ...
    while (p) {
      if (((p->pi) == pi) && (changed & (1 << (p->gpio)))) {
        ...
        if ((p->edge) ^ l) {
          if (p->ex) {(p->f)(pi, p->gpio, l, r->tick, p->user);} else {
    ...
    if ((r->flags) & PI_NTFY_FLAGS_WDOG) {
      ...
      while (p) {
        if (((p->pi) == pi) && ((p->gpio) == g)) {
          if (p->ex) {(p->f)(pi, g, PI_TIMEOUT, r->tick, p->user);} else {
      ...
      while (ep) {
        if (((ep->pi) == pi) && ((ep->event) == g)) {
          if (ep->ex) {(ep->f)(pi, g, r->tick, ep->user);} else {(ep->f)(pi, g, r->tick);}
        }
        ep = ep->next;
      }
    }
...
static void * pthNotifyThread(void * x)
{
  ...
  while (1) {
    ...
    if (bytes > 0) {got += bytes;} else {break;}
  ...
  fprintf(
    stderr, "notify thread for pi %d broke with read error %d\n",
    pi, bytes);
...
{
  if (errnum > -1000) {return cmdErrStr(errnum);} else {
    switch (errnum) {
      case pigif_bad_send:
...
int pigpio_start(const char * addrStr, const char * portStr)
{
  ...
  if (gPigCommand[pi] >= 0) {
    ...
    if (gPigNotify[pi] >= 0) {
      ...
      if (gPigHandle[pi] < 0) {return pigif_bad_noib;} else {
        ...
        *userdata = pi;
        ...
        if (gPthNotify[pi]) {return pi;} else {return pigif_notify_failed;}
...
int custom_2(
  int pi, unsigned arg1, char * argx, unsigned count,
...
int callback_cancel(unsigned id)
{
  ...
  while (p) {
    if (p->id == id) {
      ...
      if (p->prev) {p->prev->next = p->next;} else {gCallBackFirst = p->next;}

      if (p->next) {p->next->prev = p->prev;} else {gCallBackLast = p->prev;}
...
int event_callback_cancel(unsigned id)
{
  ...
  while (ep) {
    if (ep->id == id) {
      ...
      if (ep->prev) {ep->prev->next = ep->next;} else {geCallBackFirst = ep->next;}

      if (ep->next) {ep->next->prev = ep->prev;} else {geCallBackLast = ep->prev;}
