...
{
  int pwmSettingResult = set_PWM_frequency(piHandle, request->gpio_id, request->frequency);
...
TEST_F(MotorTest, EncoderCountUtils)
{
  ...
  motorOk->setEncoderCount(encoderCount);

  ASSERT_EQ(motorOk->getEncoderCount(), encoderCount);
}

TEST_F(MotorTest, GetId)
{
  ASSERT_EQ(motorOk->getId(), MOTOR_ID_1);
}

TEST_F(MotorTest, ConfigureGpios)
{
  motorOk->configureGpios(
  ...
  // TODO: Add assertion to verify GPIOs have been configured
}

TEST_F(MotorTest, SetPwmDutyCycleAndDirection)
{
  motorOk->setPwmDutyCycleAndDirection(
    motorChecker->setPwmDutycycleClient,
    0x15,
    motorChecker->setMotorDirectionClient,
    false);

  // TODO: Add assertion to verify dutycycle and direction have been set
}
